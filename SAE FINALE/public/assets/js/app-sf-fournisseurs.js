...
ready(function() {
  ...
  function ouvrirModalDetails(id) {
    ...
    const detailsHTML = `
      <div class="details-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; padding: 16px; background: #f8f9fa; border-radius: 8px;">
        <div>
    ...
    if (stats.dernieresCommandes && stats.dernieresCommandes.length > 0) {
      ...
      stats.dernieresCommandes.forEach(cmd => {
        commandesHTML += `
          <li class="commande-item" style="padding: 12px; border-bottom: 1px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center;">
            <div>
