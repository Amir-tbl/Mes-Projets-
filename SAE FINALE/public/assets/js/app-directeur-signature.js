...
function remplirModalDetails(bc, lectureSeule = false) {
  ...
  if (lectureSeule) {
    ...
    if (signaturePreview) {
      ...
      // Afficher la date et heure actuelle dans la signature
      const maintenant = new Date();
      const dateSignature = maintenant.toLocaleDateString('fr-FR', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
      ...
      if (signatureDateEl) signatureDateEl.textContent = dateSignature;
    }
  }
...
function signerBC() {
  ...
  const maintenant = new Date();
  const dateSignatureFR = maintenant.toLocaleDateString('fr-FR', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
  ...
  // Confirmation avec aperçu de la signature
  const confirmMessage = `📋 Confirmer la signature du BC ${bcSelectionne.numeroCommande} ?\n\n` +
    `✍️ Signataire : Dr. Laurent PETIT\n` +
    `📅 Date : ${dateSignatureFR}\n` +
    `💼 Fonction : Directeur de l'IUT de Villetaneuse\n\n` +
    `Le BC signé sera transmis au Service Financier pour envoi au fournisseur.`;
  ...
  if (result.success) {
    // Récupérer les commandes pour mise à jour
    ...
    if (index !== -1) {
      ...
      commandes[index].dateSignature = dateISO.split('T')[0];
      ...
      commandes[index].signataire = {
        ...
        fonction: 'Directeur de l\'IUT de Villetaneuse',
        date: dateISO,
        dateFR: dateSignatureFR,
        signature: 'Laurent Petit', // Texte de la signature
        signatureType: 'electronique'
      };

      // Créer le BC signé comme pièce jointe
      if (!commandes[index].piecesJointes) {
        commandes[index].piecesJointes = [];
      }

      // Ajouter le BC signé aux pièces jointes
      const nomBCsigne = `BC-${bcSelectionne.numeroCommande.replace('#', '')}-SIGNE.pdf`;
      commandes[index].piecesJointes.push({
        nom: nomBCsigne,
        type: 'pdf',
        categorie: 'bon_commande_signe',
        taille: '350 KB',
        url: `/uploads/bons-commande-signes/${Date.now()}-${nomBCsigne}`,
        dateAjout: dateISO,
        ajoutePar: 'Dr. Laurent PETIT - Directeur',
        description: 'Bon de commande signé électroniquement par le Directeur',
        metadonnees: {
    ...
    const messageSucces = `✅ Signature électronique effectuée avec succès !\n\n` +
      `📋 BC : ${bcSelectionne.numeroCommande}\n` +
      `✍️ Signé par : Dr. Laurent PETIT\n` +
      `📅 Date : ${dateSignatureFR}\n\n` +
      `Le BC signé a été transmis au Service Financier qui procédera à l'envoi au fournisseur.`;
